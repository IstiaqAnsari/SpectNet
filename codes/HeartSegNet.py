...
from keras.layers import Input, Conv1D, MaxPooling1D, Dense, Dropout, Flatten, Activation, AveragePooling1D, Lambda
...
def heartnet(load_path,activation_function='relu', bn_momentum=0.99, bias=False, dropout_rate=0.5, dropout_rate_dense=0.0,
    ...
    #
    input1 = Lambda(lambda x:x[:,:800,:])(input)
    input2 = Lambda(lambda x:x[:,500:1300,:])(input)
    input3 = Lambda(lambda x:x[:,1000:1800,:])(input)
    input4 = Lambda(lambda x:x[:,1500:2300,:])(input)
    input1 = Conv1D_linearphaseType(1 ,60, use_bias=False,
                    # kernel_initializer=initializers.he_normal(random_seed),
                    weights=[b1[31:]],
                    padding='same',trainable=FIR_train, type = type)(input1)
    input2 = Conv1D_linearphaseType(1, 60, use_bias=False,
                    # kernel_initializer=initializers.he_normal(random_seed),
                    weights=[b2[31:]],
                    padding='same',trainable=FIR_train, type = type)(input2)
    input3 = Conv1D_linearphaseType(1, 60, use_bias=False,
                    # kernel_initializer=initializers.he_normal(random_seed),
                    weights=[b3[31:]],
                    padding='same',trainable=FIR_train, type = type)(input3)
