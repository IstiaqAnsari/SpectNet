...
if __name__ == '__main__':
    try:
        ...
        network = 'HeartCepNet2D'
        if('2D' in network):
            from HeartCepNet import heartnet2D as heartnet
        else:
            from HeartCepNet import heartnet
        ...
        log_name = foldname +test_splitOrNot+seg[args.segment if args.segment is not None else 0]+' '+balancedOrNot+shuffledOrNot+str(int(test_split*100))+' '+ str(batch_size)+' '+str(datetime.now())+"Not Res"
        ...
        if(evaluate):
            ...
            load_path = '../../Adversarial Heart Sound Results/models/SmallNet/bcdefh_a zeropad 1 0 1000 2019-12-07 13:34:13.725294/weights.0326-0.6730.hdf5'
            paramspath = '../../Adversarial Heart Sound Results/logs/SmallNet/bcdefh_a zeropad 1 0 1000 2019-12-07 13:34:13.725294/'
            os.path.join(paramspath,'params.pickle')
            with open(os.path.join(paramspath,'params.pickle'),'rb') as f:
                loaded_params = pickle.load(f)
            print(loaded_params)
            loaded_values = []
            for k in loaded_params.keys():
                if k in keys:
                    loaded_values.append(loaded_params[k])
                else:
                    loaded_values.append(param_dict[k])

            [bn_momentum,l2_reg,l2_reg_dense,kernel_size,maxnorm,dropout_rate,dropout_rate_dense,padding,activation_function,subsam,FIR_train,trainable,optim,equ,channels] = loaded_values
            print(loaded_values)
            ...
            if('dann' in load_path):
                foldname = foldname + '_dann'
            ...
            model = heartnet(kernel_size=5,fs=fs,winlen=0.025,winstep=0.01,filters=26,random_seed=1,padding='valid',bias=False,
                       ...
                       activation_function='relu')
            model.get_layer('mfcc').trainable = False
        ...
        ####### Define Callbacks ######
            ...
            class hpRateScheduler(Callback):
                ...
                def on_epoch_begin(self, epoch,logs=None):
                    ...
                    if('grl' in [x.name for x in model.layers]):
            ...
            hprate = hpRateScheduler(sig_moid,verbose = 1)
