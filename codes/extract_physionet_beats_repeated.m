...
savedir='/media/mhealthra2/Data/heart_sound/feature/potes_1DCNN/balancedCV/folds/individual_fold_beats_repeated/';
    ...
    for row=1:ncc
        if row == ncc % for the last complete cardiac cycle
            ...
            if(isempty((tmp)))
                ...
                if(length(tmp)>2500)
                    tmp = PCG(idx_states(row,1):idx_states(row,1)+2500-1);
                end
            end
        else
        ...
        tmp_repeat = tmp;
        Nn = nsamp-length(tmp); % append zeros at the end of cardiac cycle
        while(Nn>length(tmp))
%             fprintf('File  %i  row %i N %i  tmp %i (tmp_repeat) %i\n',file_idx, row,N,length(tmp), length(tmp_repeat));
            tmp_repeat = [tmp_repeat;tmp];
            Nn = nsamp - length(tmp_repeat);
%             fprintf('File  %i  row %i N %i  tmp %i (tmp_repeat) %i\n',file_idx, row,N,length(tmp), length(tmp_repeat));
        end
        tmp_repeat = [tmp_repeat;tmp(1:Nn)];    
        disp(size(tmp_repeat));
        disp(size(x));
        x(row,1:2500) = tmp_repeat;
